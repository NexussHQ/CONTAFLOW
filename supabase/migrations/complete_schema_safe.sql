-- ========================================
-- COMPLETE SCHEMA MIGRATION FOR SUPABASE CLOUD (SAFE VERSION)
-- ========================================
-- Este archivo combina todas las migraciones con verificación de existencia
-- para evitar errores al ejecutar múltiples veces
-- ========================================

-- ========================================
-- PART 1: INITIAL SCHEMA (IF NOT EXISTS)
-- ========================================
...
CREATE TABLE IF NOT EXISTS etiquetas_ficha (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_timeline_ficha ON timeline(ficha_id);
CREATE INDEX IF NOT EXISTS idx_timeline_fecha ON timeline(fecha DESC);
CREATE INDEX IF NOT EXISTS idx_columnas_kanban_user ON columnas_kanban(user_id);
CREATE INDEX IF NOT EXISTS idx_columnas_kanban_tipo ON columnas_kanban(tipo);
CREATE INDEX IF NOT EXISTS idx_etiquetas_ficha_user ON etiquetas_ficha(user_id);

-- ========================================
-- PART 2: COMPLETE SCHEMA (IF NOT EXISTS)
-- ========================================
...
-- Añadir columna custom_values a fichas si no existe
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'fichas' AND column_name = 'custom_values'
  ) THEN
    ALTER TABLE fichas ADD COLUMN custom_values JSONB DEFAULT '{}';
  END IF;
END $$;
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Users own fichas" ON fichas;
  DROP POLICY IF EXISTS "Users own columns" ON columnas_kanban;
  DROP POLICY IF EXISTS "Users own timeline" ON timeline;
  DROP POLICY IF EXISTS "Users own tasks" ON tasks;
  DROP POLICY IF EXISTS "Users own fields" ON custom_field_definitions;
  DROP POLICY IF EXISTS "Users own etiquetas_ficha" ON etiquetas_ficha;
  DROP POLICY IF EXISTS "Users can read ficha_etiquetas" ON ficha_etiquetas;
END $$;
...
RETURNS TRIGGER AS $$
BEGIN
    -- Insertar perfil del usuario si no existe
...
-- Función para actualizar timestamp de fichas
...
-- Crear trigger para fichas si no existe
DROP TRIGGER IF EXISTS update_fichas_actualizado_at ON fichas;
