# Script de Respaldo Completo de Supabase Cloud (PowerShell)
...
supabase db dump -f "$backupDir\backup-esquema.sql" --schema-only
...
supabase db dump -f "$backupDir\backup-datos.sql" --data-only
...
supabase db dump -f "$backupDir\backup-completo.sql"
...
try {
    supabase db diff -f "$backupDir\backup-diff.sql" --use-migra
} catch {
...
supabase migration list | Out-File "$backupDir\migraciones-aplicadas.txt"
